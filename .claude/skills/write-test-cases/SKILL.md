...
- Give every `@manual` `Scenario`/`Scenario Outline` in a Feature its own title — the checklist identifies a scenario by type + title, so two sharing a title would share one Pass/Fail result, note, and screenshots. The docs build fails with a clear error if it finds a duplicate, rather than letting them silently collide.
