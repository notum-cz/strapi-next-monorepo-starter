 ...
 */
export function generateDescriptionFromTitle(title?: string): string | undefined {
...
export function generateDescriptionFromContent(
  ...
  for (const component of content) {
    ...
    if (component.__component === 'sections.hero' && 'description' in component) {
    ...
    if (component.__component === 'sections.heading-with-cta-button' && 'description' in component) {
      const desc = (component as any).description
      if (typeof desc === 'string' && desc.trim()) {
        return truncateDescription(stripHtml(desc))
      }
    ...
    if (component.__component === 'utilities.ck-editor-content' && 'content' in component) {
...
function isCommonWord(word: string): boolean {
  const commonWords = new Set([
    ...
    'may', 'might', 'must', 'can', 'page', 'home', 'main'
  ])
