...
- **Strapi MCP** is enabled by default, so an MCP-capable agent can inspect Strapi schemas and seed local content through `http://localhost:1337/mcp` with an API token.
