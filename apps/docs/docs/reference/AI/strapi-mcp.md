...
In this starter it is **enabled by default** — `mcp.enabled` in `apps/strapi/config/server.ts`, toggled with `STRAPI_MCP_ENABLED` — and exposed at `http://localhost:1337/mcp`, authenticated with a Strapi API token.
...
- **`/mcp` returns 404** — MCP is disabled; check `mcp.enabled` / `STRAPI_MCP_ENABLED` and restart Strapi.
