...
1. `.github/workflows/ci.yml` runs on push and pull requests to `main` and verifies that code builds.
2. `.github/workflows/qa.yml` is manually triggered and runs the QA tests from `qa/tests`. It should usually target a deployed UI by setting `BASE_URL`.
3. `.github/workflows/auto-pr.yml` creates or updates a PR from `dev` to `main` when changes are pushed. It extracts required environment variables from commit messages.
