...
Next.js revalidation marks the application cache stale. It does not automatically evict an upstream CDN. For incident-time CDN cache eviction, use the optional [CDN](./integrations/cdn.md) flow.
:::
...
See [CDN](./integrations/cdn.md) for the purge flow, provider setup, and Azure Front Door propagation notes.
...
Optional CDN provider variables are covered in [CDN](./integrations/cdn.md). See [Strapi environment variables](../strapi/environment-variables.md) and [UI environment variables](../ui/environment-variables.md) for the complete environment reference.
