...
The `fullpath` of every page is created by chaining the slugs of all its parents. It always starts with `/` prefix. The `fullPath` is computed by the [internal jobs system](../backend/internal-jobs.md) and **requires manual triggering** from the admin panel (see below).
...
The `fullPath` field is computed via [internal jobs](../backend/internal-jobs.md) and contains the full path of the page, including all parent slugs. It is the main identifier used to render the page from the URL (frontend finds pages using `fullPath` filter). Recalculation must be triggered manually from the admin panel.

The `fullPath` is derived from the `slug` and the `parent` relation field. The hierarchy system can be disabled on the code level (by setting `PAGES_HIERARCHY_ENABLED` to `false` in the Strapi codebase).
