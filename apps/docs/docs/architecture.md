...
Shared code lives in `packages/*` — see [Packages](./packages.md).
...
│   REST API + Documents API · Users & Permissions · i18n · Dynamic Zones      │
│   Documents-API middleware: deep populate driven by `populateDynamicZone`    │
└────────────────────────────────────┬─────────────────────────────────────────┘
                                     │
                            (server-only) STRAPI_URL
                                     │
┌────────────────────────────────────▼─────────────────────────────────────────┐
│                         Next.js Frontend (apps/ui)                           │
│                                                                              │
│   ┌─────────────────────┐                  ┌──────────────────────────┐      │
│   │  Server Components  │                  │      API proxy routes    │      │
│   │  fetch directly via │                  │  /api/public-proxy/*     │      │
│   │  PublicStrapiClient │                  │  /api/private-proxy/*    │      │
│   │  + PrivateStrapiCl. │                  │  (called from browser)   │      │
│   └─────────┬───────────┘                  └────────────┬─────────────┘      │
│             │                                           │                    │
│             └───────────────────┬───────────────────────┘                    │
│                                 ▼                                            │
│                       Strapi API Clients                                     │
│                       (BaseStrapiClient subclasses)                          │
└──────────────────────────────────────────────────────────────────────────────┘
```
...
1. **App Router match.** Next.js routes to the catch-all [`src/app/[locale]/[[...rest]]/page.tsx`](https://github.com/notum-cz/strapi-next-monorepo-starter/blob/main/apps/ui/src/app/%5Blocale%5D/%5B%5B...rest%5D%5D/page.tsx). It joins `params.rest` with `ROOT_PAGE_PATH` (`"/"` from [`@repo/shared-data`](./packages.md)) into a `fullPath`.
...
Both expose typed helpers (`fetchOne`, `fetchMany`, `fetchAll`, `fetchOneBySlug`, `fetchOneByFullPath`) generic over `UID.ContentType` from [`@repo/strapi-types`](./packages.md).
...
```ts
next: { revalidate: isDevelopment() ? 0 : 60 }
```
...
```
Browser  ──(login form)──▶  /api/auth/sign-in-strapi
                            │
                            ├──▶ Better Auth strapiAuthPlugin
                            │     └──▶ Strapi /auth/local → { jwt, user }
                            │           jwt stored in session.user.strapiJWT
...
Seed automation env: `AUTO_SEED_ENABLED`, `AUTO_SEED_MODE` — see [Data Seeding](./data-seeding.md).

## Pages Hierarchy

Pages are a parent/child tree. `fullPath` is the cached, denormalized slug chain that the catch-all route filters by. See [Pages Hierarchy](./pages-hierarchy.md).
...
- [Authentication](./authentication.md) — Better Auth + Strapi JWT details
- [Strapi API Client](./strapi-api-client.md) — client surface and proxy mechanics
- [Page Builder](./page-builder.md) — dynamic-zone → React component mapping
- [Add a Content Type](./add-content-type.md) — end-to-end workflow for a new collection
- [Packages](./packages.md) — shared `packages/*` reference
- [Pages Hierarchy](./pages-hierarchy.md) — slug/fullPath tree
- [Data Seeding](./data-seeding.md) — seed export + auto-import on dev start
