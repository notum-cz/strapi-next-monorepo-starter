...
flowchart TB
  ...
  subgraph Strapi["Strapi CMS"]
    ...
    Middleware["Document middleware<br/><code>documentMiddlewares/page.ts</code><br/>deep population rules"]:::service
    Page --> Content --> Middleware
  end
...
:::tip
You only maintain small populate objects for each component and keep them in the expected folder structure. The page middleware combines them into the full dynamic-zone populate schema automatically.
:::
